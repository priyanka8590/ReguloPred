...
def parseCommandLineArguments():
    ...
    Launch python mcl_clusters.py --help for more details
    """
    parser = argparse.ArgumentParser(prog="functional_characterization_mcl_clusters.py",description="print this")
...
def main():
    ...
    fhw.close()
    new_file = "/project/maizegdb/sagnik/bhandary/analysis_regulon_prediction/mcl_table_with_metrics.tsv"
    fhw = open(new_file, "w")
        ...
        for correlation in ["pearson", "spearman"]:
            for pca_comp in pca_comps:
                for mcl_threshold in mcl_thresholds:
                    mcl_cluster_file = options.output/correlation+"_threshold_"+pca_comp+"_"+mcl_threshold+".cluster"
                    countArray, mcl_array, genearray, random_regulon_genes, random_nonregulon_genes = mclClustersWithPathwayInfo(options, mcl_cluster_file, regulon)
...
def mclClustersWithPathwayInfo(options, mcl_cluster_file, regulon):
    countArray, random_nonphoto_gene = readDataFile(options.d)
    ...
    return countArray, mcl_array, genearray, random_photo_gene, random_nonphoto_gene
